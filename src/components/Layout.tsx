...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#E5DDF9] dark:bg-slate-950 p-2 sm:p-4 lg:p-6 font-sans text-slate-900 dark:text-slate-100 overflow-hidden">
      <div className="flex-1 flex overflow-hidden bg-[#F8F9FA] dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-white/50">
        
        {/* Sidebar */}
        <aside className="w-[88px] flex-shrink-0 bg-white dark:bg-slate-950 border-r border-slate-100 dark:border-slate-800 flex flex-col items-center py-8 z-20">
          {/* Brand Logo */}
          <div className="mb-10 relative group">
            <div className="size-12 rounded-2xl bg-gradient-to-br from-[#8B5CF6] to-[#6D28D9] flex items-center justify-center text-white shadow-lg shadow-purple-500/30 transition-transform group-hover:scale-105">
              <Store className="size-6" />
            </div>
            <div className="absolute left-full ml-4 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50">
              LotteryERP
            </div>
          </div>

          {/* Navigation Links */}
          <nav className="flex-1 flex flex-col gap-3 w-full px-4 items-center overflow-y-auto no-scrollbar">
            {navigation.map((item) => {
          ...
          {/* Bottom Actions */}
          <div className="mt-auto pt-4 flex flex-col items-center gap-3 w-full px-4">
            <div className="relative group w-full flex justify-center">
              ...
              >
                <Settings className="size-6" strokeWidth={pathname === '/configuracion' ? 2.5 : 2} />
              </Link>
              <div className="absolute left-full ml-4 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50">
                Configuración
              </div>
            ...
            <div className="relative group w-full flex justify-center">
              <button
                ...
                className="flex items-center justify-center size-12 rounded-2xl transition-all duration-300 text-slate-400 dark:text-slate-500 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20"
              >
                <LogOut className="size-6" strokeWidth={2} />
              </button>
              <div className="absolute left-full ml-4 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-red-600 text-white text-xs font-bold rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50">
                Cerrar Sesión
              </div>
            </div>
          </div>
